<template>
  <div>
    ...
    <!-- Clients posts -->
    <section class="client-posts">
      <!-- Check client posts exist -->
      <div v-if="posts.length !== 0" class="client-main">
        <div
          v-for="post in posts"
          :key="post.id"
          :post="post"
          class="client-post"
        >
          <client-widget :post="post"></client-widget>
        </div>
      </div>
    </section>
...
export default {
  ...
  async asyncData({ $prismic, params, error }) {
    try {
      ...
      const result = await $prismic.api.getSingle("clientpage", lang);
      ...
      const clientPosts = await $prismic.api.query(
        $prismic.predicates.at("document.type", "client"),
        { orderings: "[my.client.date desc]" }
      );
      // const modale
      const modaleLN = (
