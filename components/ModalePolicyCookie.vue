<template>
  <div class="bloc-modale" v-if="revelePolicy">
    <div class="overlay" @click="toggelModalePolicy"></div>
    <div class="modale-card">
      <div class="banner">
        <prismic-rich-text :field="document.data.title" />
        <button class="btn-close" @click="toggelModalePolicy">
          <svg
            version="1.1"
            id="Capa_1"
...
<script>
export default {
  name: "ModalePolicyCookie",
  props: ["revelePolicy", "toggelModalePolicy"],
  data() {
    return {
      document: {},
    };
  ...
  async fetch() {
    this.document = await this.$prismic.api.getByUID(
      "modale",
      "privacy_policy"
    );
  },
...
@media screen and (min-width: 540px) and (max-width: 767.5px)
  .modale-card,
  .banner
    left: 2rem
    right: 2rem
